...
# Definimos clases para las funciones de activación
class SigmoidActivation(object):

    @staticmethod
    def fn(z):
        # Implementado de esta forma para evitar overflows en las primeras iteraciones
        return np.where(z >= 0, 
                        1 / (1 + np.exp(-z)), 
                        np.exp(z) / (1 + np.exp(z)))
...
class Network(object):
    ...
    def __init__(self, sizes, cost=QuadraticCost, activation=ReLUActivation):
        """
    ...
    def feedforward(self, a):
        """Devuelve el output de la red siendo 'a' el input """
    ...
    def backprop(self, x, y):
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            zs.append(z)
            activations.append(self.activation.fn(z))

        # RETROPROPAGACIÓN
        # Caso base l=L
        delta = (self.cost).delta(zs[-1], activations[-1], y, self.activation.prime)
        nabla_b[-1] = delta
        nabla_w[-1] = np.dot(delta, activations[-2].transpose())
        # Caso inductivo l<L
        for l in range(2, self.num_layers):
            delta = np.dot(self.weights[-l+1].transpose(), delta) * self.activation.prime(zs[-l])
            nabla_b[-l] = delta
